...
def get_width_range_from_filename(filename):
    # Mapping based on the last character of the filename (excluding extension)
    base_name = os.path.splitext(filename)[0]
    if not base_name:
        return (20, 45) # Default
    ...
    return mapping.get(last_char, (20, 45))

def main():
    parser = argparse.ArgumentParser(description="Bulk generate AlgoArrows levels with custom width based on filename.")
    ...
    if os.path.isfile(input_path):
        ...
        output_folder = os.path.join(parent_dir, "GeneratedLevelsCustom")
    ...
    for img_name in image_files:
        ...
        try:
            ...
            min_w, max_w = get_width_range_from_filename(img_name)
            grid_width = random.randint(min_w, max_w)
            
            grid_height = int(grid_width * (orig_h / orig_w))
            grid_height = max(grid_height, 5)
            
            print(f"Processing {img_name}: Width {grid_width} (Range {min_w}-{max_w})")
